  ...
  for id in itertools::sorted(book.id_to_arit.keys()) {
    if id >= &runtime::PRECOMP_COUNT {
      ...
      if let Some(rules) = book.rule_group.get(name) {
        ...
        line(&mut fast_visit, 8, &format!("if {}_visit(ReduceCtx {{ heap, prog, tid, hold, term, visit, redex, cont: &mut cont, host: &mut host }}) {{", &build_name(name)));
  ...
  for id in itertools::sorted(book.id_to_arit.keys()) {
    if id >= &runtime::PRECOMP_COUNT {
      ...
      if let Some(rules) = book.rule_group.get(name) {
        ...
        line(&mut fast_apply, 8, &format!("if {}_apply(ReduceCtx {{ heap, prog, tid, hold, term, visit, redex, cont: &mut cont, host: &mut host }}) {{", &build_name(name)));
